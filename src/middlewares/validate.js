...
val.ValidateToken = (roles) => {
  return (req, res, next) => {
    ...
    jwt.verify(token, process.env.JWT_KEYS, (err, payload) => {
      ...
      if (err.name === "TokenExpiredError") return res.status(403).json({ pesan: "sesi telah habis,silahkan Log-In kembali" });
      if (err) return res.status(500).json({ err });
      if (roles != payload.role) return res.status(403).json({ pesan: "anda tidak memiliki akses untuk endpoint ini" });
    });
    next();
  };
};
