...
vehicleRouter.get("/", valid.Token, valid.ValidateRole(["1", "2", "3"]), controllers.getall);
// GET BY ID
...
// POST NEW VEHICLE
vehicleRouter.get("/myvehicle", valid.Token, valid.ValidateRole(["1", "3"]), controllers.getByUserId);
vehicleRouter.post("/", valid.Token, valid.ValidateRole(["1", "3"]), upload.multiUpload, controllers.create);
// DELET
vehicleRouter.patch("/:id", valid.Token, valid.ValidateRole(["1", "2", "3"]), controllers.softDelet);
// UPDATE
vehicleRouter.patch("/update/:id", upload.multiUpload, controllers.update);
// vehicle image
vehicleRouter.post("/vehicle_image/:id", valid.Token, valid.ValidateRole(["1", "3"]), upload.multiUpload, controllers.vehicleImg);
